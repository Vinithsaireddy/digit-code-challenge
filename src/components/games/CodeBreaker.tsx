    ...
    return (
      <div className="flex flex-col items-center justify-center p-6">
        ...
        <p className="text-gray-300 mb-6 text-center">
          Guess the secret color code in {maxAttempts} tries or less. After each guess, you'll get feedback on how many colors are correct and in the right position (●) or correct but in the wrong position (○).
        </p>
        <Button 
          onClick={startGame}
        ...
        {guessHistory.map((guess, index) => (
          <div key={index} className="flex items-center p-2 bg-game-dark-surface rounded-md justify-between">
            ...
            <div className="flex items-center gap-1">
              {/* Display feedback: black dots for correct position, white dots for correct color */}
              <div className="flex flex-wrap w-8 gap-1">
                {Array(guess.feedback.correct).fill(0).map((_, i) => (
                  <div key={i} className="w-2 h-2 bg-white rounded-full" />
                ))}
                {Array(guess.feedback.misplaced).fill(0).map((_, i) => (
